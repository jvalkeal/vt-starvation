...
public final class PlatformThreadClassLoadingNoStarvation {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread monitorOwner = Thread.ofPlatform().name("vt-monitor-owner").start(() -> {
		...
		System.out.println("both carriers are pinned by class-loading platform threads waiting for the ARCHIVE_MONITOR");

		RELEASE_FILE_LOCK.countDown();
		fileReader.join();
		Thread.sleep(1000);

		awaitState(loaderOne, Thread.State.TERMINATED, true);
		awaitState(loaderTwo, Thread.State.TERMINATED, true);
		System.out.println("both carriers are terminated by class-loading platform threads");
	...
	private static final class BlockingClassLoader extends ClassLoader {
		...
		BlockingClassLoader() {
			super(PlatformThreadClassLoadingNoStarvation.class.getClassLoader());
		}
